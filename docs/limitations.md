...
- **Public benchmark proof**: `benchmarks/public-repos.toml` is a curated smoke suite over real public commits, and `benchmarks/results/2026-05-26-public.md` records the current installed-wheel proof run. Treat it as a floor, not a leaderboard; expand cases before broad external claims. The synthetic sample-fixture suite is useful for regression smoke, but it is not currently a release quality gate.
...
- Keep recall gains measured with `--prove-targets`; target 60%+ recall, 50%+ token precision, and task packs within their configured budget.
