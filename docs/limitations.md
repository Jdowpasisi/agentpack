...
- **Symbol extraction**: Python (AST, full) and JavaScript/TypeScript (regex, arrow functions + classes) are strongest. Go has lightweight function, method, struct, and interface extraction. Rust, Java, and Kotlin have import graph traversal but no symbol extraction yet — they fall back to file-level summaries.
