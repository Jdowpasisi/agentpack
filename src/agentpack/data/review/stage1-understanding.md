...
3. **For each change unit, resolve what the diff does not show:**
   ...
   - **`callers`** — for every symbol this unit *exports or whose contract it changes*, find **every** call site with `rg`. List all of them by location. Inline the code snippet only for call sites whose behavior depends on a contract this unit changed; for the rest, location plus a one-line factual note is enough. Do not sample — a missed caller is a missed integration break later.
   ...
   - **`local_convention_refs`** — if the unit does something with an established analog nearby (error handling, data access, logging), point to one existing example with `path:line`. Provide the reference only; do not state whether the unit follows it.
