...
def _cursor_rule() -> str:
    commands = refresh_commands("cursor")
...
5. If MCP is unavailable, read `.agentpack/context.md`. Treat it as fallback; if its `agentpack:freshness` block says `refresh_required: true` or the task does not match, {stale_refresh} before using selected files.
...
{fallback_agent_guidance()}
{mcp_diagnostic_guidance("cursor")}
...
class CursorInstaller:
    ...
    def patch_cursor_mdc(self, root: Path) -> str:
        ...
        rule = _cursor_rule().replace("<!-- agentpack:rule:start -->\n", "").replace("\n<!-- agentpack:rule:end -->", "")
        content = f"""\
---
