...
2. Run `agentpack guard --agent codex --repair-stale --refresh-context`.
...
CLI fallback: `agentpack guard --agent codex --repair-stale --refresh-context`, `agentpack route --task "<task>"`, `agentpack pack --task auto`, then `rg` / direct file reads.
